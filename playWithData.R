...
shapesDF<-read.csv("shapes.csv") 
     ...
     #Before start lets check missing values
      ...
      any(shapesDF == 0)    # no zeros
       ...
       # max, mean and sd of area size per color
      ...
      # yeallow square average size is 3333.207
      ...
      # most likely to be green is circle with 25.83%
      ...
      #given object is red and areas size > 3000. whats the chances to any shape
      
      tab2<-subset(shapesDF,shapesDF$area>3000 & shapesDF$color=="red")
      tab2
      prop.table(summary(tab2$shape)) # answer 
      
      # Function calc the side/radius of a shape
      ...
      # We can infer that distribution of side/radius by its shape, 
